...
public class BookModelValidator implements Validator {
    ...
    public void validate(Object o, Errors errors) {
        ...
        //      s1 - код ошибки,
        //      s2 - сообщение для этой ошибки (в данном случае можно не заполнять)

        // имя
        if (model.getName() == null || model.getName().isEmpty())
            errors.rejectValue("name", "", getLocalizedMess("required_book_name"));
        // количество страниц
        if (model.getPageCount() == null)
            errors.rejectValue("pageCount", "", getLocalizedMess("required_page_count"));
        else if (model.getPageCount() < 0)
            errors.rejectValue("pageCount", "", getLocalizedMess("page_count_negative"));
        else if (model.getPageCount() == 0)
            errors.rejectValue("pageCount", "page_count_zero", getLocalizedMess("page_count_zero"));
        else if (model.getPageCount() > 5000)
        ...
        if (model.getPublishYear() == null)
            ...
            if (publishYear < 400 || publishYear > year)
